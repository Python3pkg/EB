...
def print_short_help():
    ...
    input = " usage: fastheuristic --input [(required) csv file] --outname [(required) string] " \
...
def print_extended_help():
    ...
    print('')
    print((textwrap.fill("<fastheuristic> Complete parameter list", initial_indent='  ')))
    print('')
...
class ParseArgs:
    ...
    def get_int(self, input_string):
        ...
        if input_string in ('--ensemble_size', '--ncpu'):
            ...
            if value < 0:
                print(("\n {flag} must be an integer greater than 0".format(input_string)))
                print_short_help()
                sys.exit(1)
