...
def print_short_help():
    ...
    input = " usage: slowheuristic --input [(required) csv file] --outname [(required) string] " \
            "--ensemble_size [(required) integer] --fpf [(optional) float]"
    print('')
    print((w.fill(input)))
    print('')
...
def print_extended_help():
    ...
    print('')
    print((textwrap.fill("<slowheuristic> Complete parameter list", initial_indent='  ')))
    print('')
...
def get_interface(args):
    if len(args) < 6:
        ...
        flags = ('--input', '--framework', '--ensemble_size',
                 '--ncpu', '--fpf', '--outname')
...
class ParseArgs:
    def __init__(self, args):
        self.args = args
        self.required = ('--input', '--ensemble_size', '--outname')
        self.flags = ('--input', '--ensemble_size', '--outname' '--fpf')
    ...
    def get_float(self, input_string):
        ...
        if input_string == '--fpf':
            ...
            if value > 1.0 or value < 0:
                print(("\n {flag} must be a float less than or equal to 1, e.g. 0.01".format(input_string)))
                print_short_help()
                sys.exit(1)
